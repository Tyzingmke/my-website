...
export function ContactForm() {
  ...
  useEffect(() => {
    if (!submitted) return;
    const refreshTimer = window.setTimeout(() => window.location.reload(), 3000);
  ...
  return <><form className="contact-form" onSubmit={(event) => void submit(event)}>
    ...
    <div className="contact-form-actions"><button className="button button-acid" type="submit" disabled={sending}>{sending ? <LoaderCircle className="spin" size={18} /> : <ArrowUpRight size={18} />}Send project enquiry</button>{message ? <p role="status">{message}</p> : null}</div>
  </form>{submitted ? <div className="contact-success-overlay" role="dialog" aria-modal="true" aria-labelledby="contact-success-title"><div className="contact-success-popup"><span><Check aria-hidden="true" /></span><small>Project enquiry received</small><h2 id="contact-success-title">Submitted successfully</h2><p>Check your WhatsApp or email for my update.</p><i aria-hidden="true" /></div></div> : null}</>;
}
