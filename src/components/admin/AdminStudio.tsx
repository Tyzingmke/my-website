...
function StudioCanvas({ section, document, documents, siteEvents, submissions, assets, auditEvents, members, viewport, readOnly, notice, canManageInbox, canManageAssets, canDeleteDocuments, onDeleteSubmission, onDeleteDocument, onUploadImage, onUpdate, onBodyUpdate, onSave, onPublish }: { section: AdminSection; document: CmsDocument | null; documents: CmsDocument[]; siteEvents: SiteEvent[]; submissions: FormSubmission[]; assets: Array<MediaAsset & { publicUrl: string }>; auditEvents: AuditEvent[]; members: WorkspaceMemberRecord[]; viewport: ViewportMode; readOnly: boolean; notice: string; canManageInbox: boolean; canManageAssets: boolean; canDeleteDocuments: boolean; onDeleteSubmission: (id: string) => void; onDeleteDocument: (id: string) => void; onUploadImage: (file: File) => Promise<string | null>; onUpdate: (field: "title" | "slug" | "summary" | "body", value: string) => void; onBodyUpdate: (body: CmsDocumentBody) => void; onSave: () => void; onPublish: () => void }) {
  ...
  if (!editorial) return <OperationsDashboard section={section} documents={documents} siteEvents={siteEvents} submissions={submissions} assets={assets} auditEvents={auditEvents} members={members} notice={notice} canManageInbox={canManageInbox} onDeleteSubmission={onDeleteSubmission} />;
...
function OperationsDashboard({ section, documents, siteEvents, submissions, assets, auditEvents, members, notice, canManageInbox, onDeleteSubmission }: { section: AdminSection; documents: CmsDocument[]; siteEvents: SiteEvent[]; submissions: FormSubmission[]; assets: Array<MediaAsset & { publicUrl: string }>; auditEvents: AuditEvent[]; members: WorkspaceMemberRecord[]; notice: string; canManageInbox: boolean; onDeleteSubmission: (id: string) => void }) {
