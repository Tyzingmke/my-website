...
function ThemeSettings({ document, onStudioTheme }: { document: CmsDocument | null; onStudioTheme: (theme: "dark" | "light") => void }) {
  ...
  const save = async () => {
    ...
    setSaving(false);
    setMessage(error?.message ?? data?.error ?? (data?.ok ? "Settings saved." : "Settings could not be saved."));
  };
  return <div className="studio-operations"><div className="studio-editor-heading"><div><small>Workspace settings</small><h1>Site-wide settings</h1><p>Choose the public theme used across Tony Consults.</p></div></div><section className="studio-theme-settings"><header><strong>Default public website appearance</strong><p>Dark and light image variants in every CMS block respond to this setting automatically.</p></header><div className="studio-theme-options"><button className={`studio-theme-option${theme === "dark" ? " active" : ""}`} type="button" onClick={() => selectTheme("dark")}><Moon /><strong>Dark theme</strong><small>Deep editorial background and dark-mode image assets.</small></button><button className={`studio-theme-option${theme === "light" ? " active" : ""}`} type="button" onClick={() => selectTheme("light")}><Sun /><strong>Light theme</strong><small>Bright paper surfaces and light-mode image assets.</small></button></div><footer><span>{message || (document ? "Save to publish your changes." : "The settings document is not available yet.")}</span><button className="studio-button primary" type="button" disabled={!document || saving} onClick={() => void save()}>{saving ? <LoaderCircle className="spin" /> : <Save />} Save settings</button></footer></section></div>;
}
