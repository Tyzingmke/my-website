...
function ThemeSettings({ document, onStudioTheme }: { document: CmsDocument | null; onStudioTheme: (theme: "dark" | "light") => void }) {
  ...
  const [nicheQuotes, setNicheQuotes] = useState(Array.isArray(document?.draft_body.nicheQuotes) ? document.draft_body.nicheQuotes.filter((quote): quote is string => typeof quote === "string").join("\n") : "");
  const [nichePlayQuestion, setNichePlayQuestion] = useState(String(document?.draft_body.nichePlayQuestion ?? "Wanna hug?"));
  const [nichePlayYesResponse, setNichePlayYesResponse] = useState(String(document?.draft_body.nichePlayYesResponse ?? "Yay, so when do we meet?"));
  const [nichePlayNoResponse, setNichePlayNoResponse] = useState(String(document?.draft_body.nichePlayNoResponse ?? "Wacha kukimbilia no."));
  const [saving, setSaving] = useState(false);
  ...
  const save = async () => {
    ...
    const quotes = nicheQuotes.split("\n").map((quote) => quote.trim()).filter(Boolean).slice(0, 200);
    const { data, error } = await supabase.functions.invoke("studio-cms", { body: { action: "save-document", id: document.id, version: document.version, title: document.title, slug: document.slug, draft_body: { ...(document.draft_body ?? {}), theme, nicheQuestion: nicheQuestion.trim() || "What is my reference to you?", nicheQuotes: quotes, nichePlayQuestion: nichePlayQuestion.trim() || "Wanna hug?", nichePlayYesResponse: nichePlayYesResponse.trim() || "Yay, so when do we meet?", nichePlayNoResponse: nichePlayNoResponse.trim() || "Wacha kukimbilia no." }, publish: true } });
